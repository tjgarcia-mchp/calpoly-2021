...
if args.silence_threshold != 0:
    ...
    for srcfn in sourcefiles:
        ...
        cmd = '"{0}" "{1}" "{2}" silence 0 1 {3}t {4}d : newfile : restart'
...
tmpfiles = list(
    ut.ichain(
        (iglob(srcfn.replace('.c%1n.', '.c*.')) if '.c%1n.' in srcfn else [srcfn]) 
...
dstfiles = list(
    ut.ichain(
        (iglob(srcfn.replace('.p%1n.', '.p*.')) if '.p%1n.' in srcfn else [srcfn]) 
        for srcfn in dstfiles
    )
)
